...
public class AlbumThread extends Thread {
    ...
    public void run() {
        ...
        for (int i = 0; i < numberOfRequests; i++) {
            ...
            while (!postSuccess && postRetries < MAX_RETRIES) {
                try {
                    ...
                    if (postRes.getStatusCode() == 200 || postRes.getStatusCode() == 201) {
                        ...
                        Client1.counter.incrementSuccessfulReq(1); // Count the POST request as successful
                        postSuccess = true;
                    }
            ...
            while (!getSuccess && getRetries < MAX_RETRIES) {
                try {
                    ...
                    if (getRes.getStatusCode() == 200 || getRes.getStatusCode() == 201) {
                        Client1.counter.incrementSuccessfulReq(1); // Count the GET request as successful
                        getSuccess = true;
                    }
            ...
            if (!postSuccess) {
                Client1.counter.incrementFailedReq(1); // Count POST failure as a failed request
            }

            if (!getSuccess) {
                Client1.counter.incrementFailedReq(1); // Count GET failure as a failed request
            }
        }
